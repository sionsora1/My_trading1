...
# 内置备选股票池（81只，覆盖银行/保险/白酒/新能源/医药/科技等板块龙头）
# 当数据库不可用或为空时使用此列表
_FALLBACK_STOCK_POOL = [
    # 上海（41只）
...
def load_default_stock_pool():
    """从数据库加载默认股票池，数据库为空时回退到内置备选列表"""
    try:
        ...
        db.close()
        if codes:
            print(f"[回测] 从数据库加载默认股票池: {len(codes)} 只股票")
            return codes
    except Exception as e:
        print(f"[回测] 数据库加载失败 ({e})，使用内置备选股票池")
    return _FALLBACK_STOCK_POOL.copy()


# 默认股票池（优先从数据库加载，数据库为空时使用内置备选列表）
DEFAULT_STOCK_POOL = load_default_stock_pool()


def run_backtest(stock_pool=None, start_date='20230101', end_date='20231231', max_stocks=81):
    """
    运行回测
