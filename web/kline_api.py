...
def get_pivot_points(
    ...
    # 找 trade_date 前一个交易日的日线
    prev_bar = db._conn.execute(
    ...
    if not prev_bar or not prev_bar['high'] or not prev_bar['low'] or not prev_bar['close']:
        return {'code': code, 'name': info.get('name', code),
                'pivot': None, 'error': '前一日数据缺失'}
    ...
    return {
        ...
        'support': [S1, S2, S3],
        'levels': [
            {'label': 'R3', 'price': R3, 'color': '#ef4444'},
            {'label': 'R2', 'price': R2, 'color': '#f87171'},
            {'label': 'R1', 'price': R1, 'color': '#fca5a5'},
            {'label': 'P',   'price': P,  'color': '#f59e0b'},
            {'label': 'S1', 'price': S1, 'color': '#86efac'},
            {'label': 'S2', 'price': S2, 'color': '#4ade80'},
            {'label': 'S3', 'price': S3, 'color': '#22c55e'},
        ],
    }
